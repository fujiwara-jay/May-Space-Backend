...
// Updated /bookings endpoint with availability check
app.post('/bookings', async (req, res) => {
  ...
  try {
    ...
    // Check if unit is already booked (confirmed)
    ...
    if (existingBookings.length > 0) {
      await connection.end();
      return res.status(400).json({ message: 'This unit is already booked and confirmed.' });
    }
    ...
    // If user has a denied booking, allow creating a new one
    await connection.execute(
...
// Updated booking status update with unit availability management
app.put('/bookings/:id/status', async (req, res) => {
  ...
  try {
    ...
    // If confirming a booking, check if unit is already confirmed to someone else
    if (status === 'confirmed') {
      ...
      await connection.execute(
        'UPDATE units SET is_available = ? WHERE id = ?',
        [0, unitId]
      );
    }
    ...
    if (status === 'denied' && currentStatus === 'confirmed') {
      ...
      await connection.execute(
        'UPDATE bookings SET status = ? WHERE unit_id = ? AND status = ?',
        ['denied', unitId, 'pending']
      );
    }
