...
const safeParseImages = (imagesData) => {
    ...
    try {
        ...
        // Ensure parsed result is an array before returning
        return Array.isArray(parsed) ? parsed : [];
    } catch (error) {
        // Log the error but continue execution, returning an empty array
...
app.put('/units/:id', authenticate, upload.array('images', 5), async (req, res) => {
    const unitId = req.params.id;
    const userId = req.userId;
    // NOTE: unitPrice field was missing from destructuring here
    const { buildingName, unitNumber, location, specs, specialFeatures, unitPrice, contactPerson, phoneNumber, existingImages } = req.body;
    ...
    if (!buildingName || !unitNumber || !specs || !unitPrice) { // Added unitPrice check
        return res.status(400).json({ message: 'Building Name, Unit Number, Unit Price, and Specifications are required' });
    }
    try {
        const connection = await mysql.createConnection(dbConfig);
        ...
        // Added unitPrice to the UPDATE query
        await connection.execute(
            'UPDATE units SET building_name = ?, unit_number = ?, location = ?, specifications = ?, special_features = ?, unit_price = ?, contact_person = ?, phone_number = ?, images = ? WHERE id = ? AND user_id = ?',
...
app.post('/bookings', async (req, res) => {
    ...
    try {
        ...
        const [unitRows] = await connection.execute('SELECT user_id FROM units WHERE id = ?', [unitId]); // Select only user_id needed for check
        if (unitRows.length === 0) {
            await connection.end();
            return res.status(404).json({ message: 'Unit not found' });
        }
        ...
        // NOTE: The `created_at` column is usually managed by the database, and `dateVisiting` should likely be a separate column (e.g., `visit_date`).
        // Assuming your `created_at` here is a typo and you meant a `visit_date` column for the user's requested date:
        // FIX: The query below assumes you have a column for the date/time of the visit. I've used `created_at` here to match your original INSERT, but you should adjust your schema/query if the intent was to store the requested visit date separately.
        await connection.execute(
            'INSERT INTO bookings (unit_id, user_id, name, address, contact_number, number_of_people, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
            [unitId, userId, name, address, contactNumber, numberOfPeople, dateVisiting] // The last field 'created_at' is being set by `dateVisiting`
        );
        await connection.end();
